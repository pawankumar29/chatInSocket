...
io.on('connection', (socket) => {
  ...
  //   // console.log('Received message:', socket.request);
  //   await messages.create({text:message.text,sender:socket.id})
  ...
  socket.on('chat message', async (message) => {
    // Insert the userId from req.userData into the message
    const userId:any = socket.request.userData.userId;
    await messages.create({ text: message.text, sender: userId });

    io.emit('chat message', { text: message.text, sender: userId }); // Broadcast the message to all clients
  });
